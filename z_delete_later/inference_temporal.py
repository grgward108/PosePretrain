    ...
    for epoch in range(args.num_epoch):
        for step, data in (enumerate(train_dataloader)):
            ...
            if total_steps % args.log_step == 0:
                ...
                print_str = '[Step {:d}/ Epoch {:d}] \t| L_body: {:.6f} \t L_v: {:.6f} \t L_traj: {:.6f} \t L_traj_v: {:.6f} \t lr: {:.6f} \t TOTAL: {:.6f}'. \
                    format(step+1, epoch+1, loss_rec_body.item(), loss_rec_body_v.item(), loss_rec_traj.item(), loss_rec_traj_v.item(), lr, loss.item())
