...
if __name__ == '__main__':
    ...
    output_file_path = "SAGA_traj_samples_unnormed.npy"
    # Save the array
    np.save(output_file_path, traj_samples_unnormed)

    print(f"Trajectory samples saved to {output_file_path}")
    
    
    slerp_img = generate_linear_frames_batch(joint_start_local, joint_end_local, num_frames=61, device=device)
    ...
    temporal_transformer = TemporalTransformer(
        ...
        num_joints=25, maxlen=CLIP_SECONDS * CLIP_FPS + 1
    ).to(DEVICE)
